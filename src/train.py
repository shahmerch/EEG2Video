...
SINGLE_MODEL = ("RF", {"n_estimators":200, "max_depth":10, "class_weight":"balanced"})
...
#Old data loading methodn now uses recursive parsing of All Nos folder
# ---------- DATA / FILES -----------
...
def main():
    try:
        ...
        # Iterate over each trial folder inside "All Nos"
        for trial in os.listdir(base_folder):
            trial_path = os.path.join(base_folder, trial)
            if not os.path.isdir(trial_path):
                continue
            print(f"[INFO] Processing trial folder: {trial_path}")
            for file in os.listdir(trial_path):
                if file.lower().endswith(".txt"):
                    # Assume file name format like "SM001_0_3.txt"
                    parts = file.split('_')
                    if len(parts) < 3:
                        print(f"[WARN] File name '{file}' does not match expected pattern, skipping.")
                        continue
                    try:
                        label = int(parts[1])
                    except Exception as e:
                        print(f"[WARN] Could not parse label from file '{file}', skipping. Error: {e}")
                        continue
                    full_path = os.path.join(trial_path, file)
                    X_, y_ = parse_file_group_4(full_path, label)
                    if X_.shape[0] > 0:
                        allX.append(X_)
                        allY.append(y_)
                    else:
                        print(f"[WARN] No data from file '{file}' in trial '{trial}', skipping.")

        if len(allX) == 0:
            print("[ERROR] No data loaded from any trial! Exiting.")
            return
